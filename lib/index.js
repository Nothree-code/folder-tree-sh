...
const VERSION = '0.1.1'; // keep in sync with package.json version; bumped on each release
...
export default {
  ...
  let cache = null; // { path, kind, mime, total, bytes, text }
  ...
  disposers.push(webServer.register({
    kind: 'exact',
    path: '/dsh-ftree-meta',
    handler: async (req, res) => sendJson(res, { ok: true, version: VERSION })
  }));
      ...
      try {
        ...
        if (!cache || cache.path !== path) {
          ...
          if (MIME[lower]) {
            ...
            cache = { path, kind: 'image', mime: MIME[lower], total: bytes.length, bytes, text: null };
          } else if (lower === '.pdf') {
            cache = { path, kind: 'pdf', mime: 'application/pdf', total: size, bytes: null, text: null };
          } else if (lower === '.docx') {
            const bytes = await fs.readBytes(target, undefined, MAX_FILE + 1);
            if (mammoth) {
              let html = '';
              ...
              cache = { path, kind: 'docx-html', mime: null, total: html.length, bytes: null, text: html };
            } else {
              ...
              cache = { path, kind: 'docx', mime: null, total: text.length, bytes: null, text };
            }
          } else {
            const bytes = await fs.readBytes(target, undefined, MAX_FILE + 1);
            let text;
            ...
            cache = { path, kind: 'text', mime: null, total: text.length, bytes: null, text };
          }
        }
        const c = cache;
  ...
  // GET /dsh-ftree-op?op=rename|delete|paste|open&...
  disposers.push(webServer.register({
    kind: 'exact',
    path: '/dsh-ftree-op',
    handler: async (req, res) => {
  ...
  disposers.push(webServer.register({
    ...
    handler: async (req, res) => {
      try {
        ...
        let content;
        try { content = JSON.parse(body).content; } catch { return sendJson(res, { error: 'invalid body' }); }
