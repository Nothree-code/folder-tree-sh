  ...
  disposers.push(webServer.register({
    ...
    handler: async (req, res) => {
      try {
        ...
        if (involvedResults.some((ok) => !ok)) return sendJson(res, { error: '路径不在工作区内' });
        if (op === 'rename') {
          const path = q.path;
  ...
  disposers.push(webServer.register({
    ...
    handler: async (req, res) => {
      try {
        ...
        if (shell === undefined) return sendJson(res, { error: '需要 shell 服务' });
        const roots = await workspaceRoots();
        if (!(await pathAllowed(dir, roots)) || !(await pathAllowed(target, roots))) return sendJson(res, { error: '路径不在工作区内' });
        const tq = "'" + psq(target) + "'";
        let script;
