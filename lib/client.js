window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		const promptNew = (dir, op) => {
			...
			let name = '';
			try { name = window.prompt(label + '：', def) || ''; } catch { return; }
			if (!name.trim()) return;
			apiOp({ op, path: dir, name: name.trim() }).then((res) => {
				if (res && res.ok === true) {
