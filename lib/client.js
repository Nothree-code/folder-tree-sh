window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		const renderMd = (src) => {
			...
			const out = [];
			let inCode = false, codeBuf = [], listTag = null, tbl = false;
			...
			for (let i = 0; i < lines.length; i++) {
				...
				if (h) { closeList(); closeTbl(); const lv = h[1].length; out.push('<h' + lv + '>' + mdInline(h[2]) + '</h' + lv + '>'); continue; }
				...
				if (ul) {
					...
					if (t) {
						...
						out.push('<li class="dsh-md-task' + (checked ? ' done' : '') + '"><input type="checkbox" disabled' + (checked ? ' checked' : '') + '> ' + mdInline(t[2]) + '</li>');
					} else {
				...
				if (ol) { closeTbl(); if (listTag !== 'ol') { closeList(); out.push('<ol>'); listTag = 'ol'; } out.push('<li>' + mdInline(ol[1]) + '</li>'); continue; }
				const q = line.match(/^\s*>\s?(.*)$/);
				if (q) { closeList(); closeTbl(); out.push('<blockquote>' + mdInline(q[1]) + '</blockquote>'); continue; }
				if (/^\s*\|.+\|\s*$/.test(line)) {
		...
		const CODE_LANGS = {
			...
			'.java': 'java', '.c': 'c', '.h': 'c', '.cpp': 'cpp', '.hpp': 'cpp', '.cs': 'cs', '.go': 'go', '.rs': 'rs', '.rb': 'rb', '.php': 'php'
		};
		const CODE_KEYWORDS = {
		...
		const hlCode = (text, lang) => {
			...
			const reAll = (lang === 'py' || lang === 'yaml' || lang === 'sh' || lang === 'ps1' || lang === 'rb')
				? /("(?:[^"\\\n]|\\.)*"|'(?:[^'\\\n]|\\.)*')|(#[^\n]*)/g
				: /("(?:[^"\\\n]|\\.)*"|'(?:[^'\\\n]|\\.)*')|(\/\/[^\n]*|\/\*[\s\S]*?\*\/)/g;
			const segs = [];
			let last = 0;
		...
		const doDelete = (path) => {
			showConfirm('确定删除「' + basename(path) + '」？此操作不可恢复。', () => {
				api('op', { op: 'delete', path }).then((res) => {
					if (res && res.ok === true) {
						showToast('已删除「' + basename(path) + '」');
						if (prevPath === path) closePreview();
						refreshTree();
					} else {
		...
		const doPaste = (destDir, newName) => {
			if (!clipboard) return;
			api('op', { op: 'paste', srcPath: clipboard.path, destDir, mode: clipboard.mode, newName: newName || '' }).then((res) => {
				if (res && res.ok === true) {
		...
		const doDuplicate = (path) => {
			...
			api('op', { op: 'paste', srcPath: path, destDir: dest, mode: 'copy', newName: name }).then((res) => {
				if (res && res.ok === true) {
		...
		const doOpen = (path, select) => {
			api('op', { op: 'open', path, select: select ? 'true' : 'false' }).then((res) => {
		...
		const startRename = (row) => {
			...
			const finish = (commit) => {
				...
				if (commit && val.length > 0 && val !== oldName) {
					api('op', { op: 'rename', path: oldPath, newName: val }).then((res) => {
						if (res && res.ok === true) {
							showToast('已重命名为「' + val + '」');
							refreshTree();
						} else {
		...
		const onDocClick = () => { closeMenu(); closeConfirm(); };
		...
		const pvRenderMd = () => {
			...
			art.innerHTML = renderMd(pv.text || '');
		...
		const pvEnterEdit = () => {
			...
			const mdTool = (label, title, wrap, tmpl) => {
				...
				b.addEventListener('click', (ev) => {
					...
					if (live.isConnected) live.innerHTML = renderMd(ta.value);
					ta.focus();
				});
			...
			live.innerHTML = renderMd(pv.text);
			ta.addEventListener('input', () => {
				...
				mdRenderT = setTimeout(() => { if (live.isConnected) live.innerHTML = renderMd(ta.value); }, 300);
			});
		...
		const saveMd = () => {
			...
			apiPost('write', { path }, { content }).then((res) => {
				if (!mdState.statusEl) return;
				if (res && res.ok === true) {
					mdState.dirty = false;
					const d = new Date();
		...
		function apply(ctx) {
			...
			const disposeCss = injectStyle(CSS);
					...
					disposers.push(slots.inject('shell.overlay', () => slots.register(
						...
						(props) => {
							...
							react.useEffect(() => {
								...
								setDirsSync({});
								setLoading(true);
								loadDir(wsPath).then((r) => { if (cancelled) return; setRoot(r); setLoading(false); });
							...
							react.useEffect(() => {
								...
								const stop = timerSvc.interval(() => {
									loadDir(wsPath).then((r) => setRoot((prev) => (prev && prev.path === r.path ? r : prev)));
									Object.keys(dirsRef.current).forEach((p) => {
										const v = dirsRef.current[p];
										if (v && !v.error) loadDir(p).then((r) => setDirsSync((d) => Object.assign({}, d, { [p]: r })));
									});
								}, 5000);
								return () => { if (stop) stop(); };
							}, [open, wsPath]);
							const toggleDir = (path) => {
								...
								loadDir(path).then((r) => setDirsSync((d) => Object.assign({}, d, { [path]: r })));
							};
							const rows = [];
							const walk = (entries, depth) => {
								const list = (filter ? entries.filter((e) => e.name.toLowerCase().indexOf(filter.toLowerCase()) !== -1) : entries).filter((e) => e.name.indexOf('.dshbak.') === -1);
							...
							const head = react.createElement('div', { className: 'dsh-ftree-col-head' },
								react.createElement('span', null, '📁'),
								react.createElement('span', { className: 'dsh-ftree-title' }, title),
								react.createElement('span', { className: 'dsh-ftree-path' }, wsPath || ''),
								react.createElement('span', { className: 'dsh-ftree-btn', onClick: (ev) => { ev.stopPropagation(); if (wsPath) { setRoot(null); setDirsSync({}); setLoading(true); loadDir(wsPath).then((r) => { setRoot(r); setLoading(false); }); } } }, '刷新')
							);
							...
							const body = react.createElement('div', { className: 'dsh-ftree-body' },
							...
							return react.createElement('div', { className: 'dsh-ftree-col' }, head, filterEl, body);
						}
					)));
				};
				injectSlots();
