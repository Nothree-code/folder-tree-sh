window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		const createArchiveFolderFromMenu = async (menu) => {
			const value = await promptModal('新建文件夹名称', '未分类');
			if (!value) return;
			const data = archiveLoad();
			const workspaceId = menu && menu.dataset ? (menu.dataset.dshWorkspaceId || currentWorkspaceId || '') : (currentWorkspaceId || '');
			const workspaceTitle = menu && menu.dataset ? (menu.dataset.dshWorkspaceTitle || currentWorkspaceTitle || '当前工作区') : (currentWorkspaceTitle || '当前工作区');
			const exists = data.folders.some((f) => f.name === value && (f.workspaceId || '') === workspaceId);
			if (exists) { showToast('当前工作区已存在该文件夹', 2200); return; }
			data.folders.push({ id: 'f-' + Date.now().toString(36), name: value, workspaceId, workspaceTitle, sessions: [] });
			archiveSave(data); archiveEmit(); showToast('已在「' + workspaceTitle + '」中新建文件夹「' + value + '」');
		};
		...
		const enhanceWorkspaceMenu = () => {
			...
			for (const menu of Array.from(menus)) {
				...
				const workspaceRow = anchor.closest('[role="treeitem"]') || document.querySelector('[aria-label*="工作区"]');
				if (workspaceRow) {
					const label = workspaceRow.querySelector('[class*="title"], [class*="projectText"]');
					menu.dataset.dshWorkspaceTitle = (label && label.textContent || currentWorkspaceTitle || '当前工作区').trim();
				}
				menu.dataset.dshWorkspaceId = currentWorkspaceId || '';
		...
		const renderArchiveFolders = () => {
			const data = archiveLoad();
			const folders = data.folders.filter((f) => (f.workspaceId || '') === (currentWorkspaceId || ''));
			const rows = Array.from(document.querySelectorAll('[role="treeitem"]'));
			const workspaceRow = rows.find((row) => {
				const text = (row.textContent || '').trim();
				return currentWorkspaceTitle && text.includes(currentWorkspaceTitle) && row.querySelector('[class*="projectText"], [class*="projectRow"]');
			});
			if (!workspaceRow) return;
			let host = workspaceRow.parentElement && workspaceRow.parentElement.querySelector(':scope > .dsh-ftree-archive-folders');
			if (!host) {
				host = document.createElement('div'); host.className = 'dsh-ftree-archive-folders';
				workspaceRow.parentElement.appendChild(host);
			}
			host.replaceChildren();
			for (const folder of folders) {
				const row = document.createElement('div'); row.className = 'dsh-ftree-archive-folder-row'; row.title = folder.name; row.setAttribute('role', 'treeitem');
				const icon = document.createElement('span'); icon.className = 'dsh-ftree-archive-folder-icon';
				icon.innerHTML = '<svg width="16" height="16" viewBox="0 0 16 16" fill="none" aria-hidden="true"><path d="M1.75 4.25c0-.69.56-1.25 1.25-1.25h3.02c.38 0 .74.17.98.47l.54.68h4.48c.69 0 1.25.56 1.25 1.25v5.35c0 .69-.56 1.25-1.25 1.25h-9.02c-.69 0-1.25-.56-1.25-1.25V4.25Z" fill="currentColor"/></svg>';
				const text = document.createElement('span'); text.className = 'dsh-ftree-archive-folder-label'; text.textContent = folder.name;
				const count = document.createElement('span'); count.className = 'dsh-ftree-archive-folder-count'; count.textContent = String((folder.sessions || []).length);
				row.append(icon, text, count); host.appendChild(row);
			}
			host.style.display = folders.length ? 'block' : 'none';
		}; /*
			const menus = document.querySelectorAll('[role="menu"]');
			for (const menu of Array.from(menus)) {
				...
				const injectSlots = () => {
					...
					disposers.push(slots.inject('shell.overlay', () => slots.register(
						...
						(props) => {
							...
							react.useEffect(() => { currentWsPath = wsPath; currentWorkspaceId = workspace ? workspace.workspaceId : null; currentWorkspaceTitle = title; }, [wsPath, workspace, title]);
